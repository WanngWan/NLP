...
y_pred = torch.FloatTensor([[0.3, 0.1, 0.3],
                            [0.9, 0.2, 0.9],
                            [0.5, 0.4, 0.2]])
#1、1假设正确的分类为1， 2， 0
target = torch.LongTensor([1, 2, 0])
loss = ce_loss(y_pred, target)
print("torch计算的loss：", loss)
...
def Cross_Entropy(y_pred, target):
    ...
    one_hot_target = to_one_hot(target, y_pred.shape)
    entropy = -np.sum(one_hot_target * np.log(y_pred), axis=1)
    return np.sum(entropy) / batch_size

print(Cross_Entropy(y_pred.numpy(), target.numpy()), "手动实现交叉熵")
